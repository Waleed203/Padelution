  ...
  formatEventData = (data: any[]) => {
    ...
    data.forEach((event) => {
      ...
      if (eventDate > currentDate) {
        ...
        const dateFrom = eventDate.getDate();
        const dateTo = event.days > 1 ? dateFrom + event.days - 1 : 0;
        const location = `${event.host.name}, ${event.city || event.country.name}`;
  
        let eventGroup = events.find((e: any) => e.month === monthYear);
        if (!eventGroup) {
          eventGroup = { month: monthYear, eventData: [] };
          events.push(eventGroup);
        }
  
        eventGroup.eventData.push({
          id: event.id,
          dateFrom,
          dateTo,
          title: event.name,
